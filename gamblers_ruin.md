...
simulate <- function(starting_capital, target_capital, prop, dim = 1, plot = F) {
  ...
  sim <- map(mc_results, function(df) {df[nrow(df), ]}) %>%
    bind_rows() %>%
...
![](gamblers_ruin_files/figure-gfm/1d-1.png)<!-- -->
...
![](gamblers_ruin_files/figure-gfm/2d-1.png)<!-- -->
...
![](gamblers_ruin_files/figure-gfm/3d-1.png)
